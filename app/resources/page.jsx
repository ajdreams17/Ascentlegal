...
const guides = [
  ...
  // Creator economy article — published in OC Lawyer, April 2026
  {
    slug: "creator-economy-legal-risks",
    title: "Creator Economy Legal Risks",
    description:
      "A practical look at the legal blind spots creators run into as their content businesses scale — IP ownership, brand-deal contract terms, FTC disclosure rules, and when it's time to formally structure the business.",
    readTime: "6 min read",
    badge: "Brand Deals",
    topic: "brand-deals",
    author: "By April Paredes · OC Lawyer",
    externalHref:
...
const videos = [
  {
    ...
    description:
      "Anthony breaks down what actually makes a contract enforceable, and the clauses founders most often get wrong.",
  },
  {
    id: "7EA_TbKDGq8",
    title: "The Creator Economy: What Creators Need to Know Legally",
    description:
      "April walks through the legal blind spots creators run into as their content business starts to scale.",
  },
];

// TODO: confirm exact headline/outlet framing for the Daily Journal piece — page is JS-gated
const pressMentions = [
  {
    title: "April Paredes — Daily Journal Profile",
    outlet: "Daily Journal",
    href: "https://www.dailyjournal.com/people/231101-april-paredes",
  },
];
...
function GuideCard({ g }) {
  return (
    <article className="group rounded-2xl border border-gray-100 bg-white p-6 hover:shadow-sm transition-shadow">
      ...
      {g.author && <p className="mt-2 text-xs text-gray-400">{g.author}</p>}
...
function VideoCard({ v }) {
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-4">
      ...
      {v.description && <p className="mt-1 text-sm text-gray-600">{v.description}</p>}
    </div>
  );
}
          ...
          <div className="mt-6 grid sm:grid-cols-2 gap-6">
            {pressMentions.map((p) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="block rounded-2xl border border-gray-100 bg-white p-6 hover:shadow-sm transition-shadow
                  focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-indigo-500/30"
              >
                <span className="text-xs text-indigo-600">{p.outlet}</span>
                <h3 className="mt-2 font-semibold text-gray-900">{p.title}</h3>
                <span className="mt-3 inline-block text-sm font-medium text-indigo-600">Read more →</span>
              </a>
            ))}
          </div>
        )}
